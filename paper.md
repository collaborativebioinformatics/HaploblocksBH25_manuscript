...
$y_i = \sum_{h=1}^{H}{\alpha_{ih} (x^T_{ih}U_h)}$ + $\beta_0$

where:

- $y_i$ is the phenotype of individual $i$
- $H$ is the number of haploblocks
...
For TNFa, the number of clusters is smaller within individual populations (5 in CHB, 6 in GBR and 11 in PUR) but increases to 140 when 26 populations are analyzed together. By contrast, for HLA-A, although the number of clusters within single populations is comparable to TNFa (10 in CHB, 8 in GBR and 12 in PUR), the number of clusters with single representatives across populations is much greater, considering the sequence diversity of the locus, particularly with respect to a relatively small population. These results serve as a proof of concept, showing that sequence clustering within and across haploblocks can capture differences in population structure.
...
Our ultimate objective is to perform high-resolution mapping of haploblock clusters within and across European and international biobanks, thereby providing a foundation for a systematic nomenclature of segregated genomic backgrounds and enabling the assessment of their contribution to clinical outcomes. We aim to perform a whole-genome analysis, examine the number of clusters for each haploblock and manually analyze all haploblocks that show a significantly divergent number of clusters to understand their underlying structure.
